...
class Trainer():
    ...
    def train(self):
        ...
        self.log.info('\t Loss_dis: {:.3f};\t Loss_etp: {:.3f};\t Batch time: {:.3f} sec.'.format(loss_diss, loss_en, batch_time))
    ...
    def metrics_print(self, cfg): 
        ...
        training, (training_disstortion, training_entropy, training_batch_time) = plt.subplots(3, sharex=True)
        evaluation, (eval_psnr, eval_msssim, eval_bpp, eval_test_time ) = plt.subplots(4, sharex=True)
        ...
        training_disstortion.plot(epoches, self.metrics_train_disstortion)
        training_entropy.plot(epoches, self.metrics_train_entropy)
        training_batch_time.plot(epoches, self.metrics_train_batch_time)

        eval_psnr.plot(epoches, self.metrics_eval_psnr)
        eval_msssim.plot(epoches, self.metrics_eval_msssim)
        eval_bpp.plot(epoches, self.metrics_eval_bpp)
        eval_test_time.plot(epoches, self.metrics_eval_test_time)
        
        timestamp = calendar.timegm(time.gmtime())
        training.savefig('{1}/train_{0}_.png'.format(timestamp,self.imp))
        evaluation.savefig('{1}/eval_{0}_.png'.format(timestamp,self.imp))
    ...
    def print_best_result(self, result_dict):
        ...
        self.log.info("Best image MSSIM: {:.3f}; PSNR: {:.5f}; BPP: {:.3f}; Timestamp: {:.10f}".format(best['mssim'], best['psnr'], best['bpp'], timestamp))
        self.save(timestamp, best['img'])
        
class PredefineTrainer(Trainer):
    def __init__(self, cfg, logger, model_dir, tensorboard_log_dir,imp, **kwargs):
        super(PredefineTrainer, self).__init__(cfg, logger, model_dir, tensorboard_log_dir, imp, **kwargs)
